...
class Ui_Dialog(object):
    def __init__(self):
        ...
        counter = 0
        startfile(getcwd()+"\\instalar_chromedriver.exe")
...
""))
        ...
        self.label_5.setText(_translate("Dialog", "Indique el email que tiene instalado en su teléfono móvil"))
        ...
        self.label_12.setText(_translate("Dialog", "Consulte su móvil:"))
...
"que quieras."))
        ...
        self.pushButton_AgregarMensaje.setText(_translate("Dialog", "Agregar texto"))
        self.pushButton_AdjuntarUnArchivo.setText(_translate("Dialog", "Adjuntar un archivo"))
        self.label_19.setText(_translate("Dialog", "Mensaje"))
        self.label_21.setText(_translate("Dialog", "Cliquea en cualquier mensaje para editarlo"))
        self.label_22.setText(_translate("Dialog", "Mensaje a enviar"))
        ...
        self.label_25.setText(_translate("Dialog", " No cierre la nueva ventana"))


    def log_in_ha_clickeado(self):

        response = Client.send_request(self.lineEdit_Entrar_Usuario.text(), self.lineEdit_Contrasena.text())

        if response == "GOOD":
            self.stackedWidget.setCurrentIndex(1)#Cambiamos a segunda página
            self.pushButton_ConfirmarContinuar_buscar_archivo.hide()
        elif response == "FULL MAC":
            _translate = QtCore.QCoreApplication.translate
            self.label_ErrorLogin.setText(_translate("Dialog", "Máximos dispositivos alcanzados"))
        else:
            _translate = QtCore.QCoreApplication.translate
    ...
    def is_clicked_seleccionar_archivo(self):#Aqui pilla el file location y lo pone global
        ...
        if filename:
            ...
            if filename_extension != "xlsx" and filename_extension != "csv":#Si no es xlsx da error
                self.label_que_has_seleccionado.setText(_translate("Dialog","Archivo seleccionado: {0}").format(filename))
                self.label_Error.setText(_translate("Dialog", "Error: Archivo no es .xlsx o .csv"))
            else:
                ...
                leer_xlsx = Leer_xlsx(filename, filename_extension)
    ...
    def is_clicked_confirmar_columnas(self):
        ...
        if len(nombres_nombre) != 0 and len(telefono_nombre) != 0:
            self.nombres_nombre = nombres_nombre
            self.telefono_nombre = telefono_nombre
    ...
    def is_clicked_confirmar_email(self):
        ...
        #Cogiendo el archivo cwd
        now = datetime.now()
        nombre_a_poner = str(now.year) + str(now.month) + str(now.day) + "_contactos.vcf"
        filename =  getcwd() + "\\contacts\\" + nombre_a_poner
        #------------------------

        #Introduciendo el email en el texto
        _translate = QtCore.QCoreApplication.translate
        self.label_donde_recibido_email.setText(_translate("Dialog", email))
        #----------------------email

        thread_hacer_vcard_y_transf_nombres_y_mandar_email = threading.Thread(daemon=True, target=leer_xlsx.ajustar_nombres, args=(self.nombres_nombre, self.telefono_nombre, filename, email))
        thread_hacer_vcard_y_transf_nombres_y_mandar_email.start()
        
        
    def is_clicked_confirmar_agregado_contactos(self):
        global y, contenidos_de_scroll, stop_thread, ha_scaneado
        ha_scaneado = False

        self.pushButton_confirmar_QR.hide()
    ...
    def is_clicked_confirmar_QR(self):
    ...
    def is_clicked_finalizar_componer(self):
        global telefonos, EasySend, mensaje_entero, y, stop_thread

        telefonos = leer_xlsx.return_values(self.telefono_nombre)

        #stop_thread = True
        self.poner_todo_bien()
        self.constantemente_ajustar()


        self.stackedWidget.setCurrentIndex(7)

        _translate = QtCore.QCoreApplication.translate
        self.pushButton_ENVIAR.setText(_translate("Dialog", "Enviar a ({0}) contactos").format(len(telefonos)))
...
"")
    ...
    def abrimos_whatsapp_y_esperamos(self):
        ...
        self.pushButton_confirmar_QR.show()
        _translate = QtCore.QCoreApplication.translate
        self.pushButton_confirmar_QR.setText(_translate("Dialog", "Ya he escaneado el código, continuar"))
...
"padding-bottom:10px;")
    ...
    def enviar_a_contacto(self):
        ...
        global telefonos, EasySend, mensaje_entero, driver


        self.progressBar.setMaximum(len(telefonos))
...
if __name__ == "__main__":
    ...
    Dialog = QtWidgets.QDialog(None, QtCore.Qt.WindowSystemMenuHint | QtCore.Qt.WindowTitleHint | QtCore.Qt.WindowCloseButtonHint) #Quita el question mark
    ui = Ui_Dialog()
    ui.setupUi(Dialog)
    Dialog.show()
    sys.exit(app.exec_())
